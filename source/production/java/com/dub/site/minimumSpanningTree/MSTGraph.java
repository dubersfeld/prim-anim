...
public class MSTGraph extends Graph {
	...
	public void searchStep() {
		...
		System.out.println("searchStep sator " + u.getName());
		...
		// update vertices
		System.out.println("searchStep check keys");
		for (int i1 = 0; i1 < N; i1++) {// for each index
			MSTVertex enclume = (MSTVertex)vertices[i1];
			enclume.setKey(keys[i1]);
		...
		System.out.println("searchStep return");
	}
		
	public void initPrim() {
		System.out.println("initPrim begin");
		for (int i = 1; i < N; i++) {
			keys[i] = 1000;
		}
		keys[0] = 0;
		...
		queue.display();
		System.out.println("initPrim completed");
	}
